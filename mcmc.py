...
class MCMCSampler(nn.Module):
    ...
    def sample(self, xt, model, x0hat, operator, measurement, sigma, ratio, record=False, verbose=False):
        ...
        for i in pbar:
            ...
            x = self.mc_update(x, data_term, lr, epsilon)
    ...
    def sample_finitegamma(self, xt, model, pred_xstart, operator, measurement, sigma, ratio, c_scalar, k_scalar, gamma, x_start, record=False, verbose=False):
        ...
        for i in pbar:
            ...
            JTHv = torch.autograd.grad(
                ...
                inputs=x,
                grad_outputs=(c_scalar * Hv - gamma * (k_scalar * Hx0 - measurement)),
                retain_graph=True
            )[0]

            loss = (((x + JTHv)/c_scalar) ** 2).flatten(1).sum(-1)
            ...
            # x_u_for_grad = x.detach().clone().requires_grad_()
            # early stopping with NaN
            if torch.isnan(x).any():
                return torch.zeros_like(x)
    ...
    def sample_langevin_true_energy(self, xt, model, pred_xstart, operator, measurement, sigma, lr, c_scalar, k_scalar, x_start, gamma, record=False, verbose=False):
        self.prepare_prior_score(pred_xstart, xt, model, sigma)
        x = pred_xstart.clone().detach().requires_grad_()
        ...
        pbar = tqdm(range(self.num_steps), desc="Adam Optimization") if verbose else range(self.num_steps)
        for i in pbar:
            optimizer.zero_grad()
            regularization_term = 0.5 * torch.sum(x**2)         # xxxxx 5e-2
            Hx = operator.forward(x)
            quadratic_data_term = (c_scalar / 2.0) * torch.sum(Hx**2)    # xxxxxxxx 5e-5
            inner_product_term = -torch.sum(Hx * inner_product_term_vec) # xxxxxxxx 5e-4
    ...
    def sample_finitegamma_simplified(self, xt, model, pred_xstart, operator, measurement, sigma, ratio, c_scalar, k_scalar, gamma, x_start, record=False, verbose=False):
        lr = self.get_lr(ratio)
        self.prepare_prior_score(pred_xstart, xt, model, sigma)
        ...
        for i in pbar:
            x_tmp = x.clone().detach().requires_grad_(True)
            # data_fitting_loss = ((operator.forward(x_tmp) - gamma/c_scalar * (measurement - k_scalar * operator.forward(x_start))) ** 2).flatten(1).sum(-1)
            data_fitting_loss = ((operator.forward(x_tmp) - gamma/c_scalar * measurement) ** 2).flatten(1).sum(-1)
            data_fitting_grad = torch.autograd.grad(data_fitting_loss, x_tmp)[0]

            grad = -data_fitting_grad / self.tau ** 2
            xt_term = (xt - x_tmp) / sigma ** 2
            prior_term = self.get_prior_score(x, pred_xstart, xt, model, sigma)
            grad = grad + xt_term + prior_term
            if i % 50 == 0:
