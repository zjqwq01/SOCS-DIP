...
class GaussianDiffusion:
    ...
    def p_sample_loop_sde(self,
        ...
        if record and combined_cols:
            ...
            fig.suptitle("Top: sample img | Middle: x0_t_hat | Bottom: show", fontsize=14)
...
class SDESampler(SpacedDiffusion):
    ...
    def Solve_with_CG_finitegamma_deltax(self, pred_xstart, y, operator, gamma, maxiter=100, tol=1e-5):
        device = pred_xstart.device
    ...
    def Solve_with_MCMC_finitegamma_simplified(self, model, xt, t, pred_xstart, operator, measurement, gamma):
        
        time_step = t.item()
        ratio = ((self.num_timesteps - 1 - time_step) / (self.num_timesteps - 1)) if self.num_timesteps > 1 else 1.0
        c_scalar = gamma * (1.0 - np.exp(-self.total_g2_integral))
        k_scalar = np.exp(-0.5 * self.total_g2_integral)
        x0_t_hat = self.mcmc_sampler.sample_finitegamma_simplified(xt=xt, model=model, pred_xstart=pred_xstart, operator=operator, measurement=measurement, sigma=self.sigma_steps[time_step], ratio=ratio, c_scalar=c_scalar, k_scalar=k_scalar, gamma=gamma, x_start=self.x_start)

        return x0_t_hat


    def Solve_with_CG_finitegamma(self,pred_xstart, y, operator, gamma, maxiter=100, tol=1e-5):
        device = pred_xstart.device
        dtype  = pred_xstart.dtype
        x_T_u = pred_xstart.detach().requires_grad_(True)
        Hx_T_u = operator.forward(x_T_u)

        c_scalar = gamma * (1.0 - np.exp(-self.total_g2_integral)) 

        resid_meas = y - self.alpha_T_sde * operator.forward(self.x_start.detach())
        vJp_b = torch.autograd.grad(
            outputs=Hx_T_u,
            inputs=x_T_u,
            grad_outputs=resid_meas,
            retain_graph=True
        )[0]

        # 定义 A v = v + λ J^T (H v)
        def apply_A(v_tensor: torch.Tensor) -> torch.Tensor:
            v_tensor = v_tensor.to(device=device, dtype=dtype)
            v_tensor.requires_grad_(True)
            Hv = operator.forward(v_tensor)
            JtHv = torch.autograd.grad(
                outputs=Hx_T_u,     # H(x_T^u)
                inputs=x_T_u,
                grad_outputs=Hv,
                retain_graph=True
            )[0]
        ...
        # --- SciPy CG 求解 ---
        b_numpy = vJp_b.detach().cpu().numpy().flatten()

        def matvec_for_scipy(v_flat_numpy):
            v_tensor = (torch.from_numpy(v_flat_numpy)
                        .to(device=device, dtype=dtype)
                        .view_as(pred_xstart))
            Av_tensor = apply_A(v_tensor)
            return Av_tensor.detach().cpu().numpy().flatten()

        n = int(np.prod(list(pred_xstart.shape)))
        Aop = LinearOperator(shape=(n, n), matvec=matvec_for_scipy, dtype=b_numpy.dtype)

        x_flat, info = cg(Aop, b_numpy, maxiter=maxiter, tol=tol)
        if info != 0:
            print(f"[warn] CG not converged (info={info}).")

        w = (torch.from_numpy(x_flat).to(device=device, dtype=dtype).view_as(pred_xstart))
    ...
    def p_sample(self, model, x, t, y, operator, solve_type, gamma, integral_value, mask=None, measure_config=None):
        ...
        if solve_type == 'linear':
            ...
            if measure_config['operator']['name'] in ['motion_blur_soc', 'nonlinear_blur_soc']:
                x0_t_hat = operator.transpose(y)
            elif measure_config['operator']['name'] in ['inpainting_soc','super_resolution_inpainting_soc']:
                if measure_config['mask_opt']['mask_type'] == 'random':
                    ...
                    x0_t_hat = y * mask + pred_xstart * (1-mask)
            else:
                x0_t_hat = pred_xstart - operator.transpose(operator.forward(pred_xstart)) + operator.transpose(y)
            ...
            if measure_config['operator']['name'] == 'phase_retrieval_soc':
                sample = A_t * self.x_start + B_t * x0_t_hat + sigma_t* noise
            else:
                sample = A_t * self.x_start + B_t * x0_t_hat + (sigma_t - A_t)* noise
            ...
            if t[0].item() != 0:
                ...
                x_edm_start = pred_xstart + torch.randn_like(pred_xstart) * sigma_PFODE
                pred_xstart = self.p_sample_torchdiffeq_edm(self.edm_model, x_edm_start, t, sigma_PFODE, num_steps=5, solver='euler')
            # x0_t_hat = self.Solve_with_CG_finitegamma_deltax(pred_xstart, y, operator, gamma, maxiter=300, tol=1e-3)
            # x0_t_hat = self.Solve_with_MCMC_finitegamma(model, x, t, pred_xstart, operator, y, gamma=gamma)
            # x0_t_hat = self.Solve_with_CG_finitegamma_deltax(pred_xstart, y, operator, gamma, maxiter=300, tol=1e-3)
            # x0_t_hat = self.Solve_with_CG_finitegamma(pred_xstart, y, operator, gamma, maxiter=300, tol=1e-8)
            x0_t_hat = self.Solve_with_MCMC_finitegamma_simplified(model, x, t, pred_xstart, operator, y, gamma=gamma)

            sigma_t = extract_and_expand(self.sqrt_one_minus_alphas_cumprod, t, x0_t_hat)
            noise = torch.randn_like(x0_t_hat)
            sample = (1.0 / inv_alpha_t_sde) * self.x_start + self.alpha_T_sde * (inv_alpha_t_sde - 1.0 / inv_alpha_t_sde) * x0_t_hat + sigma_t * noise
